...
class Crud_model extends CI_Model
{
    ...
    //CPMUSNI//
    function cpm_seller_sale_total($seller_id, $pay='', $pay_type=''){
        $product_price = 0;
        $tax = 0;
        $shipping = 0;
        $total = 0;
        ...
        foreach ($sales as $row) {
            $share = $this->cpm_seller_sale( $row['sale_id'], $seller_id, $pay, $pay_type, $row );
    ...
    function cpm_seller_sale($sale_id, $seller_id, $pay='', $pay_type='', $sale=''){
        $product_price = 0;
        $tax = 0;
        $shipping = 0;
        $total = 0;
        ...
        //should include fully and partially paid coz disregarding partially paid would mean disregarding transactions that you have been partially paid. but for now, not a probleem since who pay partially
        if($this->cpm_sale_payment_status('vendor', $seller_id, $sale['payment_status']) == $pay || $pay == ''){
            if($sale['payment_type'] == $pay_type || $pay_type == ''){
                if($products = $this->cpm_is_sale_of_seller($sale_id, $seller_id, $sale['product_details'])){
                    $products_in_sale = json_decode($sale['product_details'], true);
    ...
    function cpm_sale_payment_status($type='', $seller_id='', $payment_status_encoded=''){
        $payment_status = json_decode($payment_status_encoded, true);
        $paid = 0;
        $unpaid = 0;
    ...
    function cpm_is_sale_of_seller($sale_id, $seller_id, $product_details_encoded)
    {
        $output = array();
        $product_details = json_decode($product_details_encoded, true);
        foreach ($product_details as $row) {
            if ($this->cpm_is_added_by('product', $row['id'], $seller_id)) {
                $output[] = $row['id'];
            }
        }
        if (empty($output)) {
    ...
    function cpm_is_added_by($type, $id, $user_id, $user_type = 'vendor')
    {
    ...
    function cpm_get_type_name_by_id($type, $type_id = '', $field = 'name')
    {
        if ($type_id != '') {
    ...
    function cpm_seller_in_sale($sale_id){
        $vendors = $this->db->get('vendor')->result_array();
        $return = array();
        foreach ($vendors as $row) {
            if($this->cpm_is_sale_of_seller($sale_id,$row['vendor_id'])){
    ...
    function cpm_paid_to_seller($vendor_id){
        $total = 0;
    ...
    function cpm_package($courier = '', $id = 0, $status = 1){
        $package_arr = array();
        if($id > 0){
            $package_arr = $this->db->get_where('package',array('id'=>$id, 'status'=>$status, 'courier'=>$courier))->result_array();
        }
        else {
            $package_arr = $this->db->get_where('package',array('status'=>$status, 'courier'=>$courier))->result_array();
        }
        return $package_arr;
    }
    ...
    function month_total($type, $filter1 = '', $filter_val1 = '', $filter2 = '', $filter_val2 = '', $notmatch = '', $notmatch_val = '')
    {
        ...
        $a   = 0;
        if ($type == 'sale') {
            $result = $this->db->get_where('sale', array('sale_datetime >= ' => $ago,'sale_datetime <= ' => time()))->result_array();
    ...
    function sale_payment_status($sale_id,$type='',$id=''){
        $payment_status = json_decode($this->db->get_where('sale', array('sale_id' => $sale_id))->row()->payment_status,true);
        $paid = '';
        $unpaid = '';
        foreach ($payment_status as $row) {
            if($type == ''){
