...
class Home extends CI_Controller
{
	 ...
	 *	Developed by: CHRISM
	 *	Date	: 14 July, 2016
	 *	Tindahan MarketPlace
	 *	http://intelmatics.com/chrism
	 */
    ...
    function vendor($vendor_id){
        ...
        if($vendor_system	 == 'ok' && $this->db->get_where('vendor',array('vendor_id'=>$vendor_id))->row()->status == 'approved'){

            //$page_data['range']             = $min . ';' . $max;
            $page_data['range_min']         = $this->get_range_lvl('added_by', '{"type":"vendor","id":"'.$vendor_id.'"}', "min");
            $page_data['range_max']         = $this->get_range_lvl('added_by', '{"type":"vendor","id":"'.$vendor_id.'"}', "max");
    ...
    function category($para1 = "", $para2 = "", $min = "", $max = "", $text ='')
    {
        ...
        //$page_data['range']            = $min . ';' . $max;
        $page_data['range_min']        = $min;
        $page_data['range_max']        = $max;
        $page_data['text']             = $text;
        ...
        /*if ($para2 == "") {
            $page_data['all_products'] = $this->db->get_where('product', array('category' => $para1))->result_array();
        } else if ($para2 != "") {
            $page_data['all_products'] = $this->db->get_where('product', array('sub_category' => $para2))->result_array();
        }*/
        /*$page_data['category_data']    = $this->db->get_where('category', array('category_id' => $para1))->result_array();*/
        $this->load->view('front/index', $page_data);
    }
    ...
    function home_search($param = '')
    {
        $home_search = $this->input->post('home_search');
        if ($param === 'text') {
    ...
    function listed($para1 = "", $para2 = "", $para3 = "")
    {
        ...
        if ($para1 == "click") {
            ...
            if ($this->input->post('range_min')) {
                $range_min = $this->input->post('range_min');
            }
            if ($this->input->post('range_max')) {
            ...
            if (isset($range_min) && isset($range_max)) {
                $this->db->where('sale_price >=', $range_min);
                $this->db->where('sale_price <=', $range_max);
            }
            ...
            $config['first_tag_open']       = '<li><a href="#" onClick="' . $function . '; return false;">';
            ...
            $config['last_tag_open']        = '<li><a href="#" onClick="' . $function . '; return false;">';
            ...
            $config['prev_tag_open']        = '<li><a href="#" onClick="' . $function . '; return false;">';
            $config['prev_tag_close']       = '</a></li>';
            
            $function                       = "filter('click','none','none','" . ($para2 + $config['per_page']) . "')";
            $config['next_link']            = '<i class="fa fa-angle-right"></i>';
            $config['next_tag_open']        = '<li><a href="#" onClick="' . $function . '; return false;">';
            $config['next_tag_close']       = '</a></li>';
            
            $function                       = "filter('click','none','none',((this.innerHTML-1)*" . $config['per_page'] . "))";
            $config['num_tag_open']         = '<li><a href="#" onClick="' . $function . '; return false;">';
    ...
    function featured_item($min = '', $max = '')
    {
        ...
        $page_data['all_category']     = $this->db->get('category')->result_array();
        $page_data['all_sub_category'] = $this->db->get('sub_category')->result_array();
        /*$page_data['all_products']   = $this->db->get_where('product', array('featured' => "ok", 'status' => 'ok'))->result_array();
        $page_data['category_data']    = $this->db->get_where('category', array('category_id' => $para1))->result_array();*/
        $this->load->view('front/index', $page_data);
    }
    ...
    function cart_finish($para1 = "", $para2 = "")
    {
        if ($this->session->userdata('user_login') == 'yes') {
            ...
            } else if ($this->input->post('payment_type') == 'ibayad') {
                if ($para1 == 'go') {
                    ...
                    $sale_code = date('Ym', $data['sale_datetime']) . $sale_id;
                    ...
                    /****TRANSFERRING USER TO ibayad api TERMINAL****/
                    /*
                    $product_name_arr =  array();
                     foreach ($carted as $val) {
                        $product_name_arr[] = $val['name'];
                    }
                    $product_name = implode(", ", $product_name_arr);
                    */

                    $this->ibayad->add_field('merchant_reference_number', $sale_id);
                    $this->ibayad->add_field('description', $sale_code);
                    $this->ibayad->add_field('amount', $grand_total);
                    $this->ibayad->add_field('success_url', base_url() . 'index.php/home/ibayad_success');
                    $this->ibayad->add_field('fail_url', base_url() . 'index.php/home/ibayad_fail');
                    $this->ibayad->add_field('cancel_url', base_url() . 'index.php/home/ibayad_cancel');

                    // submit the fields to ibayad    
                    $this->ibayad->submit_ibayad_post();
    ...
    /* FUNCTION: Verify ibayad payment by API*/
    /*  function ibayad_success(){} */
    
    function ibayad_success(){
        
        if ($this->ibayad->is_webhook() == true) {
            if ($this->ibayad->validate_api() == true) {
                $raw_post_data = file_get_contents('php://input');
                $message_decoded = json_decode($raw_post_data);

                $payment_status_arr = array('1'=>'pending', '2'=>'paid', '3'=>'fail', '4'=>'error');
                $payment_status_id = $message_decoded->Transaction->transaction_status_id;
            
                $data['payment_details']   = $raw_post_data;
                $data['payment_timestamp'] = strtotime(date("m/d/Y"));
                $data['payment_type']      = 'ibayad';
                $sale_id                   = $message_decoded->TransactionRequest->merchant_reference_number;
                $vendors = $this->crud_model->vendors_in_sale($sale_id);
                $payment_status = array();
                foreach ($vendors as $p) {
                    $payment_status[] = array('vendor'=>$p,'status'=>$payment_status_arr[$payment_status_id]);
                }
                if($this->crud_model->is_admin_in_sale($sale_id)){
                    $payment_status[] = array('admin'=>'','status'=>$payment_status_arr[$payment_status_id]);
                }
                $data['payment_status'] = json_encode($payment_status);
                $this->db->where('sale_id', $sale_id);
                $this->db->update('sale', $data);
    ...
    function ibayad_fail(){
        $sale_id = $this->session->userdata('sale_id');
        $this->db->where('sale_id', $sale_id);
        $this->db->delete('sale');
        $this->session->set_userdata('sale_id', '');
        $this->session->set_flashdata('alert', 'payment_fail');
        redirect(base_url() . 'index.php/home/cart_checkout/', 'refresh');
    }

    function ibayad_cancel(){
        $sale_id = $this->session->userdata('sale_id');
        $this->db->where('sale_id', $sale_id);
        $this->db->delete('sale');
        $this->session->set_userdata('sale_id', '');
    ...
    function blog($para1 = "")
    {
        ...
        $num_blog   = 5;
        if($para1 == 'all'){
            $page_data['blogs'] = $this->crud_model->pagination('blog',$num_blog,'index.php/home/blog/'.$para1.'/',$f_o,$f_c,$other,$current,'4','desc');
        } else {
            $page_data['blogs'] = $this->crud_model->pagination('blog',$num_blog,'index.php/home/blog/'.$para1.'/',$f_o,$f_c,$other,$current,'4','desc',array('blog_category'=>$para1));
        }
    ...
    function invoice($para1 = "", $para2 = "")
    {
        ...
        $page_data['user_info']  = $this->db->get_where('user', array('user_id' => $this->session->userdata('user_id')))->row();
        if($para2 == 'email'){
    ...
    function get_ranger($by = "", $id = "", $start = '', $end = '')
    {
        ...
        //use json and let this stay at JS
        /*
        $return = '';
        $return .= '<script>';
        $return .= 'var range = document.getElementById("range");'; 
        ...
        $return .= '<script>';
        $return .= 'var range = document.getElementById("range");'; 
        $return .= 'range.noUiSlider.updateOptions({
                                    start: [' . $start . ',' . $end . '],
                                    range: {
                                        "min":' . $min .',
                                        "max":' . $max . '
    ...
    function get_range_lvl($by = "", $id = "", $type = "")
    {
        ...
        if (count($a = $this->db->get_where('product', array($by => $id))->result_array()) > 0) {
            foreach ($a as $r) {
                return $r['sale_price'];
            }
        } else {
    ...
    function others($para1 = "", $para2 = "", $para3 = "", $para4 = "")
    {
        if ($para1 == "get_sub_by_cat") {
            ...
            $para3 = round($para3, 0, PHP_ROUND_HALF_DOWN);
            $para4 = round($para4, 0, PHP_ROUND_HALF_UP);
            if ($para2 == 0) {
